...
void ecal_pwm_driver_set_duty_percent(pwm_phase_t phase, uint8_t duty_pct)
{
    if (phase > PWM_PHASE_W) {
        return;
    }
...
{
    ecal_pwm_driver_stop(PWM_PHASE_U);
    ecal_pwm_driver_stop(PWM_PHASE_V);
    ecal_pwm_driver_stop(PWM_PHASE_W);

    ecal_pwm_driver_set_duty_percent(PWM_PHASE_U, 0);
    ecal_pwm_driver_set_duty_percent(PWM_PHASE_V, 0);
    ecal_pwm_driver_set_duty_percent(PWM_PHASE_W, 0);
}
